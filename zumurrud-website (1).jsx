...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      const [p, l, th, s, o, acc, cats, cardDes] = await Promise.all([
        loadRemote(STORAGE_KEY_PRODUCTS, DEFAULT_PRODUCTS),
        loadRemote(STORAGE_KEY_LAYOUT, DEFAULT_LAYOUT),
        loadRemote(STORAGE_KEY_THEME, COLORS_DEFAULT),
        loadRemote(STORAGE_KEY_SETTINGS, DEFAULT_SETTINGS),
        loadRemote(STORAGE_KEY_ORDERS, []),
        loadRemote(STORAGE_KEY_ACCOUNTS, DEFAULT_ACCOUNTS),
        loadRemote(STORAGE_KEY_CATEGORIES, DEFAULT_CATEGORIES),
        loadRemote(STORAGE_KEY_CARD_DESIGN, DEFAULT_CARD_DESIGN)
      ]);
      setProducts(p); setLayout(l); setTheme(th); setSettings(s); setOrders(o); setAccounts(acc); setCategories(cats); setCardDesign(cardDes);
      setReady(true);
    })();
  }, []);

  useEffect(() => { if (ready) saveRemote(STORAGE_KEY_PRODUCTS, products); }, [products, ready]);
  useEffect(() => { if (ready) saveRemote(STORAGE_KEY_LAYOUT, layout); }, [layout, ready]);
  useEffect(() => { if (ready) saveRemote(STORAGE_KEY_THEME, theme); }, [theme, ready]);
  useEffect(() => { if (ready) saveRemote(STORAGE_KEY_SETTINGS, settings); }, [settings, ready]);
  useEffect(() => { if (ready) saveRemote(STORAGE_KEY_ORDERS, orders); }, [orders, ready]);
  useEffect(() => { if (ready) saveRemote(STORAGE_KEY_ACCOUNTS, accounts); }, [accounts, ready]);
  useEffect(() => { if (ready) saveRemote(STORAGE_KEY_CATEGORIES, categories); }, [categories, ready]);
  useEffect(() => { if (ready) saveRemote(STORAGE_KEY_CARD_DESIGN, cardDesign); }, [cardDesign, ready]);
  ...
  const ctxValue = {
    ...
    orders, setOrders, accounts, setAccounts, currentUser, setCurrentUser, showToast
  };
...
function CheckoutPage() {
  const { t, lang, cart, settings, navigate, setOrders, setCart } = useApp();
  const [form, setForm] = useState({ name: '', phone: '', address: '', governorate: '', notes: '' });
  ...
  const handleSubmit = (e) => {
    ...
    const order = {
      ...
      customer: form, items: cart, subtotal, shipping, total, status: 'pending'
    };
  ...
  if (cart.length === 0) {
    return (
      <div style={{ textAlign: 'center', padding: '100px 24px' }}>
        ...
        <button onClick={() => navigate('shop')} style={{
          marginTop: 16, background: 'var(--c-primary)', color: '#fff', border: 'none',
        ...
        {orders.map((o) => (
          <div key={o.id} style={{ background: 'var(--c-beige)', borderRadius: 12, padding: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
              ...
              <select value={o.status} onChange={(e) => updateStatus(o.id, e.target.value)} style={{ ...inp, width: 'auto' }}>
            ...
            <div style={{ fontWeight: 700, marginTop: 6 }}>{t.total}: {o.total} {lang === 'ar' ? 'ج.م' : 'EGP'}</div>
          </div>
        ))}
      </div>
    </AdminCard>
